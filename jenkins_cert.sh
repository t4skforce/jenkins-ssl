...
if [ ! -z "$ROOT_CA" ]; then
	if [ -f "${ROOT_CA}" ]; then
		if ! sudo keytool -list -keystore /etc/ssl/certs/java/cacerts -storepass changeit -noprompt -alias localrootca > /dev/null; then
			sudo keytool -keystore /etc/ssl/certs/java/cacerts -storepass changeit -noprompt -trustcacerts -importcert -alias localrootca -file ${ROOT_CA}
